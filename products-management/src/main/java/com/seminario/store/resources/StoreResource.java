...
public class StoreResource {
    ...
    public ResponseEntity<List<Store>> findAllStoresByUserId(@PathVariable("userId") Long id) {
        var stores = storeService.findAllStoresByUser(id);

        return ResponseEntity.ok(stores);
    }

    @GetMapping("/by-store-type/{storeTypeId}")
    public ResponseEntity<List<Store>> findStoresByStoreTyep(@PathVariable("storeTypeId") Long id) {
        var stores = storeService.findStoresByStoreType(id);
